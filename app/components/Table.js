...
export default class ExternalTable extends Component {
  ...
  isOverflow(){
    const theadWidth = ReactDOM.findDOMNode(this.refs.thead).clientWidth
    const tbodyWidth = ReactDOM.findDOMNode(this.refs.tbody).clientWidth
    this.setState({
                ...
                return (
                  <th
                    ...
                    <div className={cx({
                      ...
                      {
                        col.tip ? (
                          <span className="icon tip-icon" dangerouslySetInnerHTML={{__html: '&#xe7d6;'}} data-tip={col.tip}></span>
                        ) : null
                      }
                      {
                        col.sortable ? (
                          <span key={`${col.key}down`} className={cx({
                            'sort-icon icon': true,
                            'd-hide': col.key !== this.__sortParams.sortKey
  ...
  renderCell(colSetting, rowData, rowInd, colInd, rows){
    ...
    if(colSetting.format){
      if(typeof colSetting.format === 'string'){
        formatFn = this.props.format[colSetting.format]
      }else{
  ...
  render(){
    return (
      <div className={cx('table-root', 'flex-col-1', this.props.className)}>
        {this.props.showCols ? this.renderHeader() : null}
        <div className="table-body flex-col-1">
          <table className="table">
            <colgroup>
            {
              this.props.columns.map((col, colInd) => {
          ...
          {
            this.state.hasMore ? this.renderShowMore() : null
          }
        </div>
      </div>
    )
  ...
  renderNoData(){
    return (
      <tr>
        <td colSpan={this.props.columns.length}>
          <div style={{lineHeight: 100, height: 100}}>暂无数据</div>
        </td>
      </tr>
    )
  }
